...
import com.example.best_travel.infrastructure.abstract_services.ReportService;
...
public class ExcelService implements ReportService {
    ...
    private void createReport(){
        ...
        //Try with resources, cuando un objeto tiene un metodo close lo cierra automatically
        try(var outputStream = new FileOutputStream(fileLocation)) {
            workBook.write(outputStream);
            workBook.close();
        }catch (IOException ex){
            log.error("Could not be posible to create excel file", ex);
            throw new RuntimeException();
        }
    }
